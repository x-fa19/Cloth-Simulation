...
void main() {
  ...
  float h_uv = h(v_uv);
  vec2 v2_du = vec2((v_uv.x) + 1/u_texture_2_size.x, v_uv.y);
  vec2 v2_dv = vec2(v_uv.x, (v_uv.y) + 1/u_texture_2_size.y);
  ...
  vec3 phong = 0.5 * (u_light_intensity / r2) * pow(max(0, dot(nd, normalize(h))), 40);
